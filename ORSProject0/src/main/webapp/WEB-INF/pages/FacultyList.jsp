...
<style type="text/css">
body {
	background-image: url("/ORSProject0/resources/image/bg_theam.jpg");
					...
					<div align="center">
						...
						<H2>
							<span class="glyphicon glyphicon-list"></span><b><b>&nbsp;<s:message
										code="faculty.label.facultyList"></s:message>
							</b></b>
						</H2>
							...
							<div class="form-inline">
								...
								&emsp;&emsp; <label class="control-label col-sm-1"
									...
									style="padding-left: 0px; padding-top: 7px; padding-right: 0px; width: 105px;"><b><font
										color="black"><s:message code="subject.label.name"></s:message>:</font>
								</b></label>
								<div class="col-sm-2">
									...
									<sf:select STYLE="width: 100%" size="0" path="subjectId"
										...
										<sf:options items="${subjectList}" itemValue="id"
											itemLabel="subjectName"></sf:options>
									</sf:select>
								</div>
							...
							<div style="float: left">
								<div class="col-sm-3">
									<a class="btn btn-primary"
										href="<c:url value="/ctl/Faculty"></c:url>"><s:message
						...
						<c:choose>
							...
							<c:otherwise>
									...
									<table class="table  table-bordered  table-hover">
										<thead bgcolor="#9999ff">
											<tr>
												...
												<th style="text-align: center;"><s:message
														code="label.loginId" /></th>
												<th style="text-align: center;"><s:message
														code="faculty.label.mobile" /></th>

												<th style="text-align: center;"><s:message
														code="user.label.doj" /></th>
										...
										<tbody>
											...
											<c:forEach items="${list}" var="faculty" varStatus="ct">
												<tr>
													...
													<td align="center">${faculty.firstName}</td>
													<td align="center">${faculty.lastName}</td>
													<td align="center">${faculty.loginId}</td>
													<td align="center">${faculty.mobileNo}</td>
													<td align="center"><fmt:formatDate
															value="${faculty.doj }" /></td>


													<c:forEach items="${collegeList}" var="college">
														<c:if test="${faculty.collegeId==college.id}">
															<td align="center">${college.name}</td>
														</c:if>
													</c:forEach>

													<c:forEach items="${subjectList}" var="subject">
														<c:if test="${faculty.subjectId==subject.id}">
								...
								<div>
									...
									<c:choose>
										<c:when
											test="${list.size()==form.pageSize && !((facultyList.size()==form.pageSize*form.pageNo))}">
